...
class TestGetLastModifiedNoteIntegration:
    """GetLastModifiedNoteServiceとZkClientの結合テスト"""

    def test_get_last_modified_note_success_should_return_note(
        self,
        test_injector: Injector,
        mock_subprocess_run: Mock,
    ) -> None:
        # Given: 最新変更ノートのzk出力をモック
        mock_subprocess_run.return_value.stdout = (
            "/path/to/latest.md|最新変更ノート|recent,updated"
        )
        mock_subprocess_run.return_value.returncode = 0
        service = test_injector.get(GetLastModifiedNoteService)
    ...
    def test_get_last_modified_note_with_empty_tags_should_return_empty_list(
        self,
        test_injector: Injector,
        mock_subprocess_run: Mock,
    ) -> None:
    ...
    def test_get_last_modified_note_with_special_characters_should_handle_correctly(
        self,
        test_injector: Injector,
        mock_subprocess_run: Mock,
    ) -> None:
    ...
    def test_get_last_modified_note_with_pipe_in_title_should_parse_correctly(
        self,
        test_injector: Injector,
        mock_subprocess_run: Mock,
    ) -> None:
    ...
    def test_get_last_modified_note_with_zk_command_error_should_raise_runtime_error(
        self,
        test_injector: Injector,
    ...
    def test_get_last_modified_note_with_malformed_output_should_raise_error(
        self,
        test_injector: Injector,
        mock_subprocess_run: Mock,
    ) -> None:
